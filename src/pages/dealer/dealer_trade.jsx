...
export default function DealerTrade() {
  const { campana, balances, operarDealer } = useData();
  ...
  const handleSubmit = () => {
    ...
    const resultado = operarDealer(tab, cant);
    ...
    <div>
      <div className="tabar-page-header">
        ...
        <p style={{ margin: 0, color: "#8B949E", fontSize: "13px" }}>
          Mercado secundario de fardos digitales
        </p>
      </div>
      ...
      <div className="tabar-price-banner">
        ...
        <PriceItem label="Mi balance"     value={`${balances.dealer} TABAR`}  color={C.accent} />
      </div>
          ...
          <div>
            <div className="tabar-card">
              <div className="tabar-tabs">
                <button className="tabar-tab" style={tab === "buy"  ? { borderColor: "#3FB950", color: "#3FB950" } : {}} onClick={() => { setTab("buy");  setCantidad(""); setError(""); }}>Comprar</button>
                <button className="tabar-tab" style={tab === "sell" ? { borderColor: "#F85149", color: "#F85149" } : {}} onClick={() => { setTab("sell"); setCantidad(""); setError(""); }}>Vender</button>
              </div>

              {tab === "sell" && balances.dealer === 0 && (
                <div className="tabar-notice" style={{ marginBottom: "12px" }}>
                  No tenés TABAR para vender. Comprá primero desde el tab Comprar.
                </div>
              )}
              ...
              <label style={{ display: "block", fontSize: "12px", color: "#8B949E", marginBottom: "6px" }}>
                ...
                {tab === "sell" && balances.dealer > 0 && (
                  <span style={{ color: C.accent, marginLeft: "8px" }}>({balances.dealer} disponibles)</span>
                )}
              </label>
              <input
                type="number"
                min="1"
                max={tab === "sell" ? balances.dealer : campana.fardosDisponibles}
                value={cantidad}
              ...
              />
              {error && <p style={{ color: "#F85149", fontSize: "12px", margin: "6px 0 0" }}>{error}</p>}
      ...
      {step === "done" && (
        <div className="tabar-done-box">
          ...
          <p style={{ color: "#8B949E", margin: "0 0 8px", fontSize: "13px" }}>
            {tab === "buy" ? "Compraste" : "Vendiste"} {parseInt(cantidad).toLocaleString("es-AR")} TABAR por USD {total.toLocaleString("es-AR")}
          </p>
          <p style={{ color: "#484F58", margin: "0 0 20px", fontSize: "12px" }}>
            Tu balance actual: <span style={{ color: C.accent }}>{balances.dealer} TABAR</span>
          </p>
          <button onClick={() => { setStep("form"); setCantidad(""); setError(""); }} className="tabar-btn tabar-btn-secondary">
            Nueva operación
          </button>
        </div>
      )}
