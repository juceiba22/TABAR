...
export default function StateInvest() {
  ...
  const handleConfirm = () => { setError(""); setStep("confirm"); };

  const handleSubmit = () => {
    const resultado = invertirState(tokensEstimados);
    if (!resultado.ok) { setError(resultado.error); setStep("form"); return; }
    setStep("done");
  };
    ...
    <div>
      <div className="tabar-page-header">
        ...
        <p style={{ margin: 0, color: "#8B949E", fontSize: "13px" }}>
          Participación institucional en la campaña TABAR
        </p>
      </div>
        ...
        <div className="tabar-layout-sidebar">
          <div>
            <div className="tabar-card">
              ...
              <button
                ...
                disabled={!campana.activa || !monto || tokensEstimados <= 0}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            <div className="tabar-notice">
              El rendimiento del 8.5% es anualizado y se aplica sobre la duración efectiva de la campaña.
            </div>
          </div>
        </div>
      )}
      ...
      {step === "done" && (
        <div className="tabar-done-box">
          ...
          <h2 style={{ color: "#3FB950", margin: "0 0 8px", fontSize: "20px" }}>Inversión confirmada</h2>
          <p style={{ color: "#8B949E", margin: "0 0 20px", fontSize: "13px" }}>
            Acreditamos {tokensEstimados.toLocaleString("es-AR")} TABAR en tu balance FET. Podés ver tus rendimientos en la sección correspondiente.
          </p>
          <button onClick={() => { setStep("form"); setMonto(""); setError(""); }} className="tabar-btn tabar-btn-secondary">
            Nueva inversión
          </button>
        </div>
      )}
