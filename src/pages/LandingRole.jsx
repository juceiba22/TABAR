/**
 * LandingRole.jsx — TABAR Auth v2
 *
 * Fixes aplicados vs versión anterior:
 *  ① Perfil Firestore se crea en primer login verificado, no en registro
 *  ② Códigos de error Firebase v9+ (auth/invalid-credential)
 *  ③ Pantalla dedicada "verificá tu email" con reenvío y countdown
 *  ④ Redirect seguro: sin rol → pantalla "acceso pendiente" en vez de /undefined
 *  ⑤ Eliminado setTimeout artificial del finally
 *  ⑥ CSS extraído de dangerouslySetInnerHTML → clases reales
 *  ⑦ Show/hide contraseña
 *  ⑧ Confirmación de contraseña en registro
 *  ⑨ Strength indicator de contraseña
 */
...
export default function LandingRole() {
  ...
  useEffect(() => {
    ...
    // Usuario autenticado pero sin rol → pantalla "sin acceso" manejada abajo
  ...
  const validateRegister = useCallback(() => {
    ...
    return null;
  }, [displayName, companyName, password, passwordConfirm]);
...
/* ④ Pantalla para usuarios autenticados sin rol asignado */
...
/* ═══════════════════════════════════════════════════════════════════════════
   Estilos — ⑥ Extraídos del dangerouslySetInnerHTML original y expandidos
   Nota: en un proyecto real, mover a tabar-responsive.css y eliminar este bloque.
═══════════════════════════════════════════════════════════════════════════ */
const STYLES = `
/* ── Root ────────────────────────────────── */
.lr-root {
